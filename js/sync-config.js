(function(){
  ...
  const C = ROOT.config = Object.freeze({
    ...
    PROD_WARNING: 'TEST BUILD — uses isolated browser storage and a TEST Google Drive appData file.'
  });
    ...
    stable(v){
      ...
      const sort=x=>{
        ...
        if(x && typeof x==='object'){
          ...
          const o={}; Object.keys(x).sort().forEach(k=>{ if(x[k]!==undefined) o[k]=sort(x[k]); }); return o;
        }
        return x;
      };
